"""
    ...
    I took the implementation of apriori from github and used it as an example to try and write my
    own implementation. Then I realized that my version was WAYYYY more complicated than this version.
    So much so that I got confused looking at it. So, I am tweaking this version that is online to 
    fit the rule finding/printing that we need to do.
...
# Reads a file. Returns a list of tuples (transactions) and each unique word
def readFile(fileName):
	...
	for row in reader:
		...
		if firstPass == False: 
			transaction = []
			for word in row:
				word = headings[i] + word
				transaction.append(word)
				itemSet.add(frozenset([word]))	# Generate 1-itemsets
				if word in words:	# Count frequencies of all words
					words[word] += 1
				else:
					words[word] = 1
				i += 1
...
def getItemSetTransactionList(data_iterator):
    """Take a generator and return an itemSet set and list of all transactions"""
        ...
        for item in value:
            ...
            for element in _subsets:
                ...
                if len(remain)>0:
                    ...
                    rawConfidence = str(getRawSupport(item)) + "/" + str(getRawSupport(element))
                    if confidence >= minConfidence and confidence < 1.0:
...
def printResults(items, rules):
    ...
    print "\n------------------------ RULES:"
    for rule, confidence, rawConfidence in rules:
        pre, post = rule
        print "Rule: %s ==> %s , %.3f (%s)" % (str(pre), str(post), confidence, rawConfidence)
